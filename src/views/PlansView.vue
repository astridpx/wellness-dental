...
const { errorMessage, fetchPlans, loadingPlans, plans } = usePlans()

type PlanFilters = {
  planTypeId: string
  planCode: string
  dentalPremium: string
}
...
const filters = reactive<PlanFilters>(emptyFilters())
const draftFilters = reactive<PlanFilters>(emptyFilters())

const filteredPlans = computed(() => {
  const planTypeId = filters.planTypeId.trim().toLowerCase()
  const planCode = filters.planCode.trim().toLowerCase()
  const dentalPremium = filters.dentalPremium.trim().toLowerCase()

  return plans.value.filter(
    (plan) =>
      (!planTypeId || String(plan.PlanTypeID).toLowerCase().includes(planTypeId)) &&
      (!planCode || plan.PlanCode.toLowerCase().includes(planCode)) &&
      (!dentalPremium || String(plan.DentalPrem1).toLowerCase().includes(dentalPremium)),
  )
})
    ...
    <template #dialog-content>
      <div class="space-y-5">
        ...
        <div class="grid gap-5">
          <AppInput
            v-model="draftFilters.planTypeId"
            label="Plan Type ID"
            placeholder="e.g. 951"
            icon="feather:hash"
          />
          <AppInput
            v-model="draftFilters.planCode"
            label="Plan Code"
            placeholder="e.g. XNT003"
            icon="feather:tag"
          />
          <AppInput
            v-model="draftFilters.dentalPremium"
            label="Dental Premium 1"
            placeholder="e.g. 8"
  ...
  <div class="space-y-6">
    ...
    >
      <div class="grid gap-6 p-6 sm:p-8 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
        ...
        <div
          class="flex items-center gap-4 rounded-[1.5rem] border border-pebble bg-white/85 px-5 py-4 shadow-sm"
        >
          <span
            class="flex h-11 w-11 items-center justify-center rounded-2xl bg-emerald-light text-emerald"
          >
      ...
      <div class="grid gap-px border-t border-pebble bg-pebble sm:grid-cols-2">
        <div class="bg-white px-6 py-5">
          ...
          <AppStatValue :loading="loadingPlans" :value="plans.length" />
        </div>
        <div class="bg-white px-6 py-5">
          ...
          <div class="mt-2 flex items-center gap-2 text-sm font-semibold text-onyx">
            <Icon
              v-if="loadingPlans"
              icon="feather:loader"
              class="h-4 w-4 animate-spin text-slate"
            />
            <span v-else class="h-2.5 w-2.5 rounded-full bg-emerald" />
            {{ loadingPlans ? 'Loading plans' : 'Available for viewing' }}
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="rounded-[1.5rem] border border-pebble bg-white p-5 shadow-sm sm:p-6">
      ...
      <div v-else class="overflow-hidden rounded-[1.5rem] border border-pebble">
        <AppTable
          :theads="['Plan Type ID', 'Plan Code', 'Dental Premium 1']"
          :total-entries="filteredPlans.length"
          :total-pages="1"
          :current-page="1"
        >
          <template #trs>
            <tr v-for="plan in filteredPlans" :key="plan.PlanTypeID">
              <td>
      ...
      <div
        class="mt-4 flex items-start gap-3 rounded-2xl border border-pebble bg-cloud px-4 py-3 text-sm text-slate"
      >
